...
public class Controller {
	...
	public ReConfigResultCollector reConfigResultCollector = new ReConfigResultCollector();
	...
	private boolean initComplete = false;// 是否已经完成了初始化
...
//		}
	...
	public void handle(ReConfigSignal signal){
		if(signal.getType() == ReConfigSignal.ReConfigSignalType.INIT && !initComplete){
			...
			if(upStreamToWait == 0 && reConfigToWait == 0){
				...
				try {
					...
					rescaleNum = Integer.valueOf(prop.getProperty("rescale"));
						...
						try {
							...
							while (true) {
								...
								if(json.containsKey("increase") || json.containsKey("migrate")){
									...
									CompletableFuture<Acknowledge> acknowledgeCompletableFuture = reConfigResultCollector.startNewReConfigAction(
										reConfigAttemptSet,
										tmp);
...
//			}
			reConfigResultCollector.acknowledgeReConfig(
...
//			}
			try {
				...
				for (int batch = 0; batch < currentSplitNum; ++batch) {
					acknowledgeCompletableFuture = reConfigResultCollector.startNewReConfigAction(
						tmp,
						version.addAndGet(1));
					partitionManager.migrte(
						entry,
...
//	}
	...
	public void mayBeRescale(long tmpID){
		if(tmpID >= 700L && !triggerRescale){
			...
			rescaleManager.rescale("ReConfig", 2);
		}
	}
	...
	public void updateReConfigVertex(ExecutionVertex[] reConfigExecutionVertex){
		System.out.println("reConfigToWait:"+reConfigToWait + " reConfigExecutionVertex:"+reConfigExecutionVertex.length+" "+reConfigExecutionVertex);
		this.reConfigExecutionVertex = reConfigExecutionVertex;
		this.reConfigToWait = reConfigExecutionVertex.length;
		reConfigAttemptSet.clear();
		for(ExecutionVertex ev : reConfigExecutionVertex){
			if(ev == null){
				continue;
			}
			reConfigAttemptSet.add(ev.getCurrentExecutionAttempt().getAttemptId());
		}
	}
...
//					tmpp,
//					version.addAndGet(1));
//				partitionManager.migrte(0, 1, 4, 0, 1, version.get());
//				acknowledgeCompletableFuture.get();
//				cleanStage();
//
//				tmpp = new HashSet<>();
...
//					tmpp,
//					version.addAndGet(1));
//				partitionManager.migrte(0, 1, 5, 0, 1, version.get());
//				acknowledgeCompletableFuture.get();
//				cleanStage();
//
//				tmpp = new HashSet<>();
...
//					tmpp,
//					version.addAndGet(1));
//				partitionManager.migrte(0, 1, 7, 0, 1, version.get());
//				acknowledgeCompletableFuture.get();
//				cleanStage();
//
//				tmpp = new HashSet<>();
//				tmpp.add(reConfigExecutionVertex[0].getCurrentExecutionAttempt().getAttemptId());
//				tmpp.add(reConfigExecutionVertex[2].getCurrentExecutionAttempt().getAttemptId());
//				acknowledgeCompletableFuture = reConfigResultCollector.startNewReConfigAction(
//					tmpp,
//					version.addAndGet(1));
//				partitionManager.migrte(0, 2, 8, 0, 1, version.get());
//				acknowledgeCompletableFuture.get();
//				cleanStage();
//
//				tmpp = new HashSet<>();
...
//					tmpp,
//					version.addAndGet(1));
//				partitionManager.migrte(0, 2, 9, 0, 1, version.get());
//				acknowledgeCompletableFuture.get();
//				cleanStage();
//
//				tmpp = new HashSet<>();
...
//					tmpp,
//					version.addAndGet(1));
//				partitionManager.migrte(0, 2, 10, 0, 1, version.get());
//				acknowledgeCompletableFuture.get();
//				cleanStage();

//				tmpp = new HashSet<>();
//				tmpp.add(reConfigExecutionVertex[2].getCurrentExecutionAttempt().getAttemptId());
//				tmpp.add(reConfigExecutionVertex[4].getCurrentExecutionAttempt().getAttemptId());
//				acknowledgeCompletableFuture = reConfigResultCollector.startNewReConfigAction(
//					tmpp,
//					version.addAndGet(1));
//				partitionManager.migrte(2, 4, 31, 0, 1, version.get());
//				acknowledgeCompletableFuture.get();
//				cleanStage();
...
//						tmpp = new HashSet<>();
//						tmpp.add(reConfigExecutionVertex[0].getCurrentExecutionAttempt().getAttemptId());
//						tmpp.add(reConfigExecutionVertex[3].getCurrentExecutionAttempt().getAttemptId());
//						acknowledgeCompletableFuture = reConfigResultCollector.startNewReConfigAction(
//							tmpp,
//							version.addAndGet(1));
//						System.out.println("start migrate keyGroup:"+entry.getKey()+" on:"+entry.getValue());
//						partitionManager.migrte(0, 3, entry.getKey(), 0, 1, version.get());
//						acknowledgeCompletableFuture.get();
//						cleanStage();
//					}
//				}
//
				rescaleManager.rescale("ReConfig", 4);
				...
				acknowledgeCompletableFuture = reConfigResultCollector.startNewReConfigAction(
					tmpp,
					version.addAndGet(1));
				partitionManager.migrte(0, 3, 8, 0, 1, version.get());
				acknowledgeCompletableFuture.get();
				cleanStage();
				...
				acknowledgeCompletableFuture = reConfigResultCollector.startNewReConfigAction(
					tmpp,
					version.addAndGet(1));
				partitionManager.migrte(0, 3, 9, 0, 1, version.get());
				acknowledgeCompletableFuture.get();
				cleanStage();
				...
				acknowledgeCompletableFuture = reConfigResultCollector.startNewReConfigAction(
					tmpp,
					version.addAndGet(1));
				partitionManager.migrte(0, 3, 10, 0, 1, version.get());
				acknowledgeCompletableFuture.get();
				cleanStage();
				...
				acknowledgeCompletableFuture = reConfigResultCollector.startNewReConfigAction(
					tmpp,
					version.addAndGet(1));
				partitionManager.migrte(1, 3, 19, 0, 1, version.get());
				acknowledgeCompletableFuture.get();
				cleanStage();
				...
				acknowledgeCompletableFuture = reConfigResultCollector.startNewReConfigAction(
					tmpp,
					version.addAndGet(1));
				partitionManager.migrte(1, 3, 20, 0, 1, version.get());
				acknowledgeCompletableFuture.get();
				cleanStage();
				...
				acknowledgeCompletableFuture = reConfigResultCollector.startNewReConfigAction(
					tmpp,
					version.addAndGet(1));
				partitionManager.migrte(1, 3, 21, 0, 1, version.get());
				acknowledgeCompletableFuture.get();
				cleanStage();
				...
				acknowledgeCompletableFuture = reConfigResultCollector.startNewReConfigAction(
					tmpp,
					version.addAndGet(1));
				partitionManager.migrte(2, 3, 30, 0, 1, version.get());
				acknowledgeCompletableFuture.get();
				cleanStage();
				...
				acknowledgeCompletableFuture = reConfigResultCollector.startNewReConfigAction(
					tmpp,
					version.addAndGet(1));
				partitionManager.migrte(2, 3, 31, 0, 1, version.get());
				acknowledgeCompletableFuture.get();
				cleanStage();
//
//
//				Thread.sleep(60000);
//				rescaleManager.rescale("ReConfig", 7);
//
			}catch (Exception e){
				e.printStackTrace();
			}
