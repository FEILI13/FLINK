...
public class JobCausalLogImpl implements JobCausalLog {
	...
	// Stores the logs to be shared downstream (regarding determinant sharing depth), in a hierarchical fashion以分层方式存储要在下游共享的日志(关于决定共享深度)
	...
	public void registerTask(VertexGraphInformation vertexGraphInformation,
		...
		for (ResultPartitionWriter writer : resultPartitionsOfLocalVertex) {
			...
			long partitionIDLower = intermediateResultPartitionID.getIntermediateResultPartitionID().getLowerPart();
			long partitionIDUpper = intermediateResultPartitionID.getIntermediateResultPartitionID().getUpperPart();
