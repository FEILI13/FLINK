...
public class MigrateManager<N, K, S> {
	...
	public void migrate(long version, int keyGroupIndex, int batch, Map<N, Map<K, S>> map) throws IOException {
		//RedisUtil.set(keyGroupIndex+"-"+batch, JSON.toJSONString(map));
		//System.out.println("now migrating");
		byte[] key = SerializerUtil.serialize(version+"-"+keyGroupIndex+"-"+batch);
		if(map == null){
	...
	public Map<N, Map<K,S>> fetchState(long version, int keyGroupIndex, int batch) throws IOException, InterruptedException {
		//return JSON.parseObject(RedisUtil.get("test" + i), new TypeReference<Map<N, Map<K,S>>>() {});
		byte[] key = SerializerUtil.serialize(version+"-"+keyGroupIndex+"-"+batch);
