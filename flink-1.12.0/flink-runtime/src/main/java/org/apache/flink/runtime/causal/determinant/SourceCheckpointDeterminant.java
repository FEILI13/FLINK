...
public class SourceCheckpointDeterminant extends AsyncDeterminant {
	...
	public void process(RecoveryManagerContext recoveryManagerContext) {
		CompletableFuture<Long> latestBytesProcessedDuringAlignment = new CompletableFuture<>();
		latestBytesProcessedDuringAlignment.complete(0l);
		CompletableFuture<Long> duration = new CompletableFuture<>();
		duration.complete(0l);
		CheckpointMetricsBuilder metrics = new CheckpointMetricsBuilder()
			...
			.setBytesPersistedDuringAlignment(0)
			.setAlignmentDurationNanos(duration);
