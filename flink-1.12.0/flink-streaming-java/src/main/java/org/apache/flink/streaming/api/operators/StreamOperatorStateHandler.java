...
public class StreamOperatorStateHandler {
    ...
    public void migrate(int keyGroupIndex, int batch, int splitNum) {
		if(keyedStateBackend != null){
			try {
				CheckpointableKeyedStateBackend rawBackend = keyedStateBackend;
				rawBackend.migrate(keyGroupIndex, batch, splitNum);
			}catch (Exception e){
				e.printStackTrace();
				throw new RuntimeException("Exception occur in migrate state");
			}
		}
    }
	...
	public void fetchState(int keyGroupIndex, int batch, int splitNum) {
		if(keyedStateBackend != null){
			try {
				CheckpointableKeyedStateBackend rawBackend = keyedStateBackend;
				rawBackend.fetchState(keyGroupIndex, batch, splitNum);
			}catch (Exception e){
